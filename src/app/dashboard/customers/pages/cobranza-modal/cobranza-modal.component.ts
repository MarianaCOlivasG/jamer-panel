...
export class CobranzaModalComponent implements OnInit {
  ...
  @Input() workOrderFolio!: string;
  @Input() saldo: number = 0;
  @Input() bWorkOrder!: any;
  ...
  async create( ) {
    ...
    try {
      const resp = await this.customersPaymentsService.addPayment( { payment: this.form.value } ).toPromise();
      ...
      if ( this.file ) {
        await this.uploadsService.uploadCobranza( resp.payId, this.file ).toPromise();
      }
